...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    for weight in shuffledWeights:
        astarrX = AStar(heuristic_type, weight, max_nr_states_to_expand)
        res = astarrX.solve_problem(problem)
        if res.is_solution_found:
            weightsWithSolution.append(weight)
...
def monetary_cost_objectives_mda_problem_experiments():
    ...
    uc1 = UniformCost()
    res = uc1.solve_problem(small_mda_problem_with_monetary_cost)
    print(res)

    res = uc1.solve_problem(moderate_mda_problem_with_monetary_cost)
    print(res)
