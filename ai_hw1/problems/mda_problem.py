...
class MDAProblem(GraphProblem):
    """
    An instance of this class represents an MDA problems.
    """
    ...
    def get_operator_cost(self, prev_state: MDAState, succ_state: MDAState) -> MDACost:
        ...
        if isinstance(succ_state.current_site, Junction):
            ...
            succ_junction = succ_state.current_site
        else:
            # succ_junction_index = succ_state.current_site.location.index
            succ_junction = succ_state.current_site.location

        cost_distance = self.map_distance_finder.get_map_cost_between(prev_junction, succ_junction)
    ...
    def get_all_certain_junctions_in_remaining_ambulance_path(self, state: MDAState) -> List[Junction]:
        ...
        """
        currentLocation=state.current_site if isinstance(state.current_site,Junction) else state.current_site.location
        return sorted([apartment.location for apartment in self.get_reported_apartments_waiting_to_visit(state)].append(state.current_site.location),key=lambda x: x.index)
