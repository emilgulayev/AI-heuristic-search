...
class MDAMaxAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        return max(
            self.cached_air_distance_calculator.get_air_distance_between_junctions(junction1, junction2) for junction1
            in self.problem.get_all_certain_junctions_in_remaining_ambulance_path(state)
            for junction2 in self.problem.get_all_certain_junctions_in_remaining_ambulance_path(state) if
            junction1.index != junction2.index)
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        """
        junctionsGraph = nx.Graph()
        # junctionsGraph.add_nodes_from([junction for junction in junctions])
        # junctionsGraph.add_weighted_edges_from([])
